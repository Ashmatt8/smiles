import { Atividade } from '../atividade-model';

export const AtividadesCorAzul: Atividade[] = [
    {
        id: 1,
        descricao: 'Qual a cor da baleia?',
        imagem: 'assets/images/Desenhos/icone_Baleia.png',
        resposta: 4,
        opcoes: [
            {
                id: 1,
                descricao: 'Verde',
                imagem: 'assets/images/Desenhos/icone_Verde.png'
            },
            {
                id: 2,
                descricao: 'Vermelho',
                imagem: 'assets/images/Desenhos/icone_Vermelho.png'
            },
            {
                id: 3,
                descricao: 'Amarelo',
                imagem: 'assets/images/Desenhos/icone_Amarelo.png'
            },
            {
                id: 4,
                descricao: 'Azul',
                imagem: 'assets/images/Desenhos/icone_Azul.png'
            }
        ]
    },
    {
        id: 2,
        descricao: 'Qual a cor da borboleta?',
        imagem: 'assets/images/Desenhos/icone_Borboleta.png',
        resposta: 2,
        opcoes: [
            {
                id: 1,
                descricao: 'Amarelo',
                imagem: 'assets/images/Desenhos/icone_Amarelo.png'
            },
            {
                id: 2,
                descricao: 'Azul',
                imagem: 'assets/images/Desenhos/icone_Azul.png'
            },
            {
                id: 3,
                descricao: 'Verde',
                imagem: 'assets/images/Desenhos/icone_Verde.png'
            },
            {
                id: 4,
                descricao: 'Vermelho',
                imagem: 'assets/images/Desenhos/icone_Vermelho.png'
            }
        ]
    },
    {
        id: 3,
        descricao: 'Marque a bolinha azul:',
        resposta: 3,
        opcoes: [
            {
                id: 1,
                imagem: 'assets/images/Desenhos/icone_Vermelho.png'
            },
            {
                id: 2,
                imagem: 'assets/images/Desenhos/icone_Verde.png'
            },
            {
                id: 3,
                imagem: 'assets/images/Desenhos/icone_Azul.png'
            },
            {
                id: 4,
                imagem: 'assets/images/Desenhos/icone_Amarelo.png'
            }
        ]
    },
    {
        id: 4,
        descricao: 'Qual desenho é da cor azul?',
        imagem: 'assets/images/Desenhos/icone_Azul.png',
        resposta: 4,
        opcoes: [
            {
                id: 1,
                descricao: 'Coração',
                imagem: 'assets/images/Desenhos/icone_Coração.png'
            },
            {
                id: 2,
                descricao: 'Árvore',
                imagem: 'assets/images/Desenhos/icone_Arvore.png'
            },
            {
                id: 3,
                descricao: 'Sol',
                imagem: 'assets/images/Desenhos/icone_Sol.png'
            },
            {
                id: 4,
                descricao: 'Baleia',
                imagem: 'assets/images/Desenhos/icone_Baleia.png'
            }
        ]
    }
  ];
